...
public class ExcelView extends AbstractView {
    ...
    protected void buildExcelDocument(Map model, XSSFWorkbook wb, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        System.out.println("------엑셀 실행이 여기까지 올까요??---------------");
...
//        List<EgovMap> dataList = (List<EgovMap>) dataMap.get("list"); // 데이터가 담긴 리스트 //7
...
//            	dataEgovMap = dataList.get(i);
                // 맨 앞 컬럼인 "번호"는 idx라는 이름으로 여기서 생성하여 넣어준다.
//                dataEgovMap.put("idx", (i+1)+""); 
//            	List<WorkVO> dataOneValue = new ArrayList<WorkVO>();
//            	dataOneValue.add(dataList.get(i));
                
                for(int j=0; j<columnVarArr.length; j++){
//                    String data = (String) dataEgovMap.get(columnVarArr[j]);
                    String data = (String) dataList.get(i).get(columnVarArr[j]);
                    System.out.println(dataList.get(i).get(columnVarArr[j]));
